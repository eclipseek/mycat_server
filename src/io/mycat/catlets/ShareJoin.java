...
// 执行成功，但改成：
// /*!mycat:catlet=io.mycat.catlets.ShareJoin */ select a.id as xxx, b.id as hostid, b.host_ip from hps_cfg_label  a  join  hps_host b on b.id=a.id;
// 执行失败，这是 1.6 版本的 bug，2.0 版本已经修复该问题。
public class ShareJoin implements Catlet {
	...
	public void processSQL(String sql, EngineCtx ctx) {
		// 原始 sql = select a.id as id, b.id as hostid, b.host_ip from hps_cfg_label  a  join  hps_host b on b.id=a.id
		// 这里 ssql = select id as id from hps_cfg_label
		// 分析：join 查询分两次查询来做，第一步，先查询第一个表 hps_cfg_label，这里 ssql 就是解析出来的查询 hps_cfg_label 表的 sql。
   ...
   public void endJobInput(String dataNode, boolean failed){
	...
	private void createQryJob(int batchSize) {	
		...
		*/
		...
		// 原始 sql = /*!mycat:catlet=io.mycat.catlets.ShareJoin */ select a.id as id, b.id as hostid, b.host_ip from hps_cfg_label  a  join  hps_host b on b.id=a.id;
		// 这里 sql = select id, id as hostid , host_ip from hps_host where id in (1,2,3)
		//
		// 分析：join 查询分两步：
		// 第一步：先查询 hps_cfg_label 表，查询这个表所有记录的 id 字段值（1，2，3）；
		// 第二部：查询出的 id 值，作为查询 hps_host 表的参数，构成 sql = select id, id as hostid , host_ip from hps_host where id in (1,2,3)
