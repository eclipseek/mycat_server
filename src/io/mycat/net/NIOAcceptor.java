...
public final class NIOAcceptor extends Thread  implements SocketAcceptor{
	...
	public NIOAcceptor(String name, String bindIp,int port, 
		...
		// xman. 注册OP_ACCEPT，监听客户端连接
		this.serverChannel.register(selector, SelectionKey.OP_ACCEPT);
		// xman. FrontendConnectionFactory, 用来封装channel成为 FrontendConnection
	...
	private void accept() {
		...
		try {
			...
			// xman. 利用NIOProcessor管理前端链接，定期清除空闲连接，同时做写队列检查
			NIOProcessor processor = MycatServer.getInstance().nextProcessor();
			c.setProcessor(processor);

			// xman. 和具体执行selector响应感兴趣事件的NIOReactor绑定
