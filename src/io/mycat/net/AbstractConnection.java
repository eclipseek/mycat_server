...
public abstract class AbstractConnection implements NIOConnection {
	...
	public void onReadData(int got) throws IOException {
		...
		// xman. 读取到的字节小于0，表示流关闭，如果等于0，代表TCP连接关闭了
		if (got < 0) {
		...
		for (;;) {
			...
			// xman. 如果postion小于包起始位置加上包长度，证明readBuffer不够大，需要扩容
			if (position >= offset + length && readBuffer != null) {
				...
				// xman. 处理包，每种AbstractConnection的处理函数不同
				handle(data);
				...
				// reached end
				// xman. 如果最后写入位置等于最后读取位置，则证明所有的处理完了，可以清空缓存和offset
				// 否则，记录下最新的offset 由于readBufferOffset只会单线程（绑定的RW线程）修改，但是
				// 会有多个线程访问（定时线程池的清理任务），所以设为volatile，不用CAS
				if (position == offset) {
	...
	public final void write(ByteBuffer buffer) {
		...
		if (isSupportCompress()) {
			...
			// zhangyq ~ 2017/07/30/17:41 ~ at 江宁河定桥 STARBUCKS ~ 消息写入 writeQueue
			// 利用缓存队列和写缓冲记录保证写的可靠性。
			// 当前线程会立刻从该队列中取出来发送，假如因某些原因无法发送，
			// 定时线程会也会将其发送出去。
			writeQueue.offer(buffer);
		}
	...
	/* zhangyq ~ 16:12/2017-08-02 ~ 大数据产业基地亚信2楼 ~ 写 ByteBuffer 前判断空间是否足够写
	 * 空间不够依据 writeSocketIfFull 采取两种策略：
	 * 1. buffer 写入writeQueue，重新分配一块 capacity 大小的；
	 * 2. 重新分配一块 capacity + buffer.limit 大小的空间，将 buffer 中数据放入，返回新 ByteBuffer；
	 * */
	...
	public void onConnectfinish() {
		LOGGER.debug("连接后台真正完成，连接信息 ---> " + this.toString());
	}	
}
