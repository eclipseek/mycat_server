...
public abstract class FrontendConnection extends AbstractConnection {
	...
	public FrontendConnection(NetworkChannel channel) throws IOException {
		...
		// zhangyq ~ 2017/07/30/17:36 ~ at 江宁河定桥 STARBUCKS ~ 设置认证 handler
		// 初始的前端连接，handler 都是 FrontendAuthenticator，因为要做握手认证。
	...
	// 关于握手包的封装，可以参考：http://www.bitscn.com/pdb/mysql/201407/226725.html "服务器的握手包" 部分。
	@Override
	public void register() throws IOException {
		if (!isClosed.get()) {
			...
			// zhangyq ~ 2017/07/30/17:38 ~ at 江宁河定桥 STARBUCKS ~ 发送握手报文
			// 服务端向客户端发送握手报文
			hs.write(this);

			// zhangyq ~ 2017/07/30/16:59 ~ at 江宁河定桥 STARBUCKS ~ 尝试读取响应消息
			// 握手报文发送后，尝试一次同步读取认证报文，如果读取不到也没关系，
			// RW 线程中会去读取。
			this.asynRead();
		}
	}
