 ...
 * @author mycat
 *
 * xman.作为客户端去连接后台数据库（MySql，后端NIO通信）
 */
			...
			try {
				...
				try {
					for (SelectionKey key : keys) {
						// xman. 通过加调试得知，这里的 att，是从 MySQLConnectionFactory.make 方法创建的 MySQLConnection c 传过来的。
	...
	private void connect(Selector selector) {
		...
		while ((c = connectQueue.poll()) != null) {
			try {
				...
				// xman. 注册 connect. 并传入 c，作为 key 的 attachment
