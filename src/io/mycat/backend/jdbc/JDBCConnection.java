...
public class JDBCConnection implements BackendConnection {
	...
	// zhangyq | 2017/08/02/13:08 | 大数据产业基地亚信2楼 | 调用存储过程
	...
	/* zhangyq ~ 16:26/2017-08-02 ~ 大数据产业基地亚信2楼 ~ ResultSet 封装成标准 Mysql 协议并写入 ByteBuffer
	 * 1. 写入 ResultSetHeaderPacket
	 * 2. 写入 FieldPacket
	 * 3. 写入 EOFPacket
	 * 4. 写入 RowDataPacket
	 * 5. 写入 EOFPacket
	 *
	 * 参考：https://my.oschina.net/alchemystar/blog/834150
	 * */
    private void ouputResultSet(ServerConnection sc, String sql)
		...
		try {
			...
			while (rs.next()) {
				...
				byteBuf = curRow.write(byteBuf, sc, false);
				byteBuf.flip();
				/* zhangyq P ~ 16:41/2017-08-02 ~ 大数据产业基地亚信2楼 ~ ByteBuffer 数据转入byte[]
				* 是为了减少 ByteBuffer 的分配。
				* */
				byte[] row = new byte[byteBuf.limit()];
				byteBuf.get(row);
				byteBuf.clear();
	...
	public void execute(final RouteResultsetNode node,
		...
		/* zhangyq ~ 17:58/2017-07-31 ~ 大数据产业基地亚信2楼 ~ SQL 语句由业务线程异步处理 */
