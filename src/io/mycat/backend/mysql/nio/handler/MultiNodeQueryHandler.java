...
public class MultiNodeQueryHandler extends MultiNodeHandler implements LoadDataResponseHandler {
	...
	public void execute() throws Exception {
		...
		// zhangyq | 2017/08/02/14:30 | 大数据产业基地亚信2楼 | 在每个目标节点上执行 sql 语句
	...
	public void rowEofResponse(final byte[] eof, BackendConnection conn) {
		...
		/* zhangyq ~ 19:39/2017-08-02 ~ 大数据产业基地亚信2楼 ~ 最后一个分片 */
		if (decrementCountBy(1)) {
			...
			if (dataMergeSvr != null) {
				try {
					/* zhangyq ~ 19:39/2017-08-02 ~ 大数据产业基地亚信2楼 ~ 加入报文结束标志
					 * 加入该标志（AbstractDataNodeMerge.END_FLAG_PACK）是为了让 Merge 线程知道
					 * 所有分片数据都已获取完毕。
					 * */
